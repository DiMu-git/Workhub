...
public interface FacilitiesRepository extends JpaRepository<Facilities,String> , JpaSpecificationExecutor<Facilities> {
    ...
    @Query(value="select * from facilities where ((start_date<=?1 and end_Date>=?1) or (start_date<=?2 and end_date>=?2) or (start_date>=?1 and end_date<=?2)) and name like ?3",nativeQuery=true)
    List<Facilities> getByDate( String startDate, String endDate,String name);
