...
public interface ActivitiesRepository extends JpaRepository<Activities,String> , JpaSpecificationExecutor<Activities> {
    ...
    @Query(value="select * from activities where ((start_date<=?1 and end_Date>=?1) or (start_date<=?2 and end_date>=?2) or (start_date>=?1 and end_date<=?2)) and name like ?3",nativeQuery=true)
    List<Activities> getByDate(String startDate, String endDate,String name);
