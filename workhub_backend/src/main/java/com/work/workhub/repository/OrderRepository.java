...
public interface OrderRepository extends JpaRepository<Orders,String> , JpaSpecificationExecutor<Orders> {
    ...
    @Query(value="SELECT COUNT(venue_name)  \n" +
            "FROM orders\n" +
            "where time =?1 ",nativeQuery=true)
    Integer timeCount(String time);
    ...
    @Query(value="SELECT ROUND(sum(price),2) \n" +
            "FROM orders\n" +
            "where time =?1",nativeQuery=true)
    Integer timePay(String time);

    @Query(value="SELECT count(*) from orders where time >=?1 and time <=?2 ",nativeQuery=true)
    Integer total(String startDate,String endDate);

    @Query(value="SELECT ROUND(sum(price),2) from orders where time >=?1 and time <=?2 ",nativeQuery=true)
    Double payAll(String startDate,String endDate);
}
