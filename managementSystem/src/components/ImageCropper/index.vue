...
export default {
  props: {
    // Domain, upload file name, trigger event will take (if a page multiple picture upload control, can be distinguished
    field: {
      type: String,
      default: 'avatar'
    },
    // Name key, similar to ID, trigger event will take (if a page multiple picture upload control, can be distinguished
    ki: {
      type: Number,
      default: 0
    },
    // Displays whether the control is displayed
    value: {
      type: Boolean,
      default: true
    },
    ...
    // Other files to be uploaded with data, object format
    params: {
      type: Object,
      default: null
    },
    ...
    // Crop the picture to height
    height: {
      type: Number,
      default: 200
    },
    // Rotation is not displayed
    noRotate: {
      type: Boolean,
      default: true
    },
    ...
    // Whether cross-domain is supported
    withCredentials: {
      type: Boolean,
      default: false
    }
  ...
  data() {
    ...
    return {
      ...
      // Steps
      step: 1, // 1 Select the file. 2 Cut the file. 3 Upload the file
      // Upload status and progress
      loading: 0, // 0 Not started 1 In progress 2 Succeeded 3 Error
      progress: 0,
      // Whether there are errors and error messages
      hasError: false,
      errorMsg: '',
      // Requirement diagram aspect ratio
      ratio: width / height,
      // Original map address, generated picture address
      sourceImg: null,
      sourceImgUrl: '',
      createImgUrl: '',
      ...
      sourceImgMouseDown: {
        on: false,
        mX: 0, // Coordinates of the mouse down
        mY: 0,
        x: 0, // Scale Original coordinates
        y: 0
      },
      // The size of the container to generate the image preview
      previewContainer: {
        width: 100,
        height: 100
      },
      ...
      sourceImgContainer: {
        // sic
        width: 240,
        height: 184 // A bug occurs if the graph scale is the same as this
      },
      // The original image shows the properties
      scale: {
        zoomAddOn: false, // Button zoom event enabled
        zoomSubOn: false, // Button zoom event enabled
        range: 1, // Maximum 100
        rotateLeft: false, // Button left rotation event enabled
        rotateRight: false, // Button right rotation event enabled
        degree: 0, // Rotating degree
        x: 0,
        y: 0,
        width: 0,
        height: 0,
  ...
  computed: {
    ...
    sourceImgStyle() {
      ...
      return {
        ...
        transform: 'rotate(' + scale.degree + 'deg)', // Left original image rotation style when rotating
        '-ms-transform': 'rotate(' + scale.degree + 'deg)', // Compatible with ie 9
        ...
        '-o-transform': 'rotate(' + scale.degree + 'deg)' // Compatible with the Opera
      }
    },
    ...
    sourceImgMasking() {
      ...
      return {
        scale, // Scale the mask relative to the required width and height
        x,
        y,
        width: w,
        height: h
  ...
  created() {
    // Bind the esc key to hide this plug-in event
  ...
  methods: {
    ...
    // Preparation work before cutting
    startCrop() {
      const {
        width,
        height,
    ...
    // Mouse down the picture ready to move
    imgStartMove(e) {
      e.preventDefault()
      // If touch events are supported, mouse events are invalid
    ...
    // The picture moves when the mouse is pressed down
    imgMove(e) {
      e.preventDefault()
      // If touch events are supported, mouse events are invalid
      if (this.isSupportTouch && !e.targetTouches) {
    ...
    // The button is pressed to begin rotation to the right
    startRotateRight(e) {
      const { scale } = this
    ...
    // The button is pressed to begin rotation to the left
    startRotateLeft(e) {
      const { scale } = this
    ...
    // Release or remove button to cancel magnification
    endZoomAdd(e) {
    ...
    // Button release or remove to cancel narrowing
    endZoomSub(e) {
      const { scale } = this
    ...
    zoomImg(newRange) {
      ...
      // New coordinates (scale according to mask center point)
      let nX = sWidth / 2 - (nWidth / width) * (sWidth / 2 - x)
      let nY = sHeight / 2 - (nHeight / height) * (sHeight / 2 - y)
      // Determines whether the new coordinates exceed the mask limit
      if (nX > 0) {
        nX = 0
      }
      if (nY > 0) {
    ...
    createImg(e) {
      ...
      if (e) {
        // Cancel the mouse press and move state
        this.sourceImgMouseDown.on = false
      }
      canvas.width = this.width
      canvas.height = this.height
      ctx.clearRect(0, 0, this.width, this.height)
      // Set the transparent area to white bottom
      ctx.fillStyle = '#fff'
